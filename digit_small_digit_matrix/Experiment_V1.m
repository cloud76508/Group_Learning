...
for fold = 1:5
    ...
    learnNeg(:,(fold-1)*8*800+1:fold*8*800) = [];
    
    learnPos = trainPositiveData;
    learnPos(:,(fold-1)*8*800+1:fold*8*800) = [];
    
    learn.X = [learnNeg'; learnPos'];
    learn.y = [-ones(size(learnNeg,2),1);ones(size(learnPos,2),1)];
    
    valNeg = trainNegativeData(:,(fold-1)*8*800+1:fold*8*800);
    valPos = trainPositiveData(:,(fold-1)*8*800+1:fold*8*800);
    val.X = [valNeg'; valPos'];
