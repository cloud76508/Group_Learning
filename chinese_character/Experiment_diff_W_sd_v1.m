...
window_size_list = [10,15,20,25,30,35,40,45,50,55];
...
%     [~, ~, decValNeg] = predict(-ones(size(valNegativeData,2),1), sparse(valNegativeData'), model);
    ...
    for n =1:size(testPositiveList,1)
        ...
        [number_windows,testPositiveData] = partition_sd_v1(temp_data.character,window_size);
    ...
    for n =1:size(testNegativeList,1)
        ...
        [number_windows,testNegativeData] = partition_sd_v1(temp_data.character,window_size);
