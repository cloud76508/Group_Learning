...
def GoupData(rdata, gs, n1, n2):
    ...
    data_GL = np.zeros(shape=(int(rdata.shape[0]*rn*cn), gs, gs))
    k = 0
    for i in range(rdata.shape[0]):
        for j in range(rn):
            for q in range(cn): 
                data_GL[k] = rdata[i][(j*30):(j*30+gs),q*30:(q*30+gs)]
                k = k+1
    label_GL = np.concatenate((np.ones([n1*cn*rn,1]),np.zeros([n2*cn*rn,1])), axis = 0)
